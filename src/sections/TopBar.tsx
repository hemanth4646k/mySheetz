      ...
      <div className="px-2 flex gap-[4px] items-center text-sm">
        {breadcrumbItems.map((b, index) => 
            <div key={index} className={`flex items-center leading-[20px] ${index == breadcrumbItems.length - 1 ? "gap-[8px]" : "text-[#AFAFAF]"}`}>
              <div className="p-1 pb-1.5 cursor-pointer">{b.label}</div>
              <img 
                className={`${index == breadcrumbItems.length - 1 ? "w-[24px] h-[12px] cursor-pointer" : "w-[12px] h-[12px]"}`} 
  ...
  return (
    <div className="flex gap-1 items-center">
      <div className="flex rounded-[6px] items-center gap-[8px] h-[40px] w-[165px] tracking-wide bg-[#F6F6F6] text-[#757575] p-[12px] text-xs">
        ...
        <input 
          className="flex items-center h-[16px] border-0 focus:outline-none bg-[#F6F6F6] w-full" 
          placeholder="Search within sheet"
        />
      </div>
      <div className="flex items-center justify-center w-[40px] h-[40px]">
        <img className="w-full h-full cursor-pointer" src="icons/Notification_bell.svg" alt="Notification_bell" />
      </div>
      <ProfileBlock />
    </div>
  )
...
function ProfileBlock(){
  return(
    <div className="flex items-center gap-3 pl-2 h-[40px] cursor-pointer focus-within:bg-gray-100 hover:bg-gray-200 transition-colors duration-200 ease-in-out rounded-xl">
      <img 
      ...
      <div className="flex flex-col">
        <div className="text-sm font-medium text-gray-900 leading-tight max-w-[80px] truncate">
          John Doe
        </div>
        <div className="text-xs text-gray-500 leading-tight max-w-[80px] truncate">
